...
_REPLACEMENTS = {
    "\u2014": "-",   # em dash
    "\u2013": "-",   # en dash
    "\u2011": "-",   # non-breaking hyphen (AI used this in "Cross-Site")
    "\u2012": "-",   # figure dash
    "\u2192": "->",  # right arrow
    "\u2018": "'", "\u2019": "'",   # smart single quotes
    "\u201c": '"', "\u201d": '"',   # smart double quotes
    ...
    "\u2022": "-",    # bullet
    "\u00a0": " ",    # non-breaking space
    "\u202f": " ",    # narrow no-break space (AI used this between number+word)
    "\u2009": " ",    # thin space
    "\u200b": "",     # zero-width space
}
...
def clean(text) -> str:
    """Make any value safe for fpdf2 core fonts, and strip markdown emphasis."""
    if text is None:
        return ""
    s = str(text)
    # Strip markdown bold/italic markers the LLM sometimes emits (**text**, *text*).
    s = s.replace("**", "")
...
def load_json(path, default):
    # Guard against the known "config.json becomes a directory" Docker bug,
    # and any other IO/parse error — never crash the whole report over config.
    if os.path.isdir(path):
        print(f"[WARN] {path} is a directory (Docker bind-mount bug), using default")
        return default
    try:
        with open(path) as f:
            return json.load(f)
...
def build_executive_summary(pdf: ReportPDF, meta, active, passive, totals, ai):
    ...
    if ai and ai.get("executive_summary"):
        ...
        pdf.multi_cell(pdf.w - pdf.l_margin - pdf.r_margin, 4.5,
                       clean("Summary generated by the AI module."),
...
def build_annex(pdf: ReportPDF):
    ...
    pdf.body_text(
        ...
        "vulnerabilities.\n"
        "- An optional AI module that generates contextual explanations "
